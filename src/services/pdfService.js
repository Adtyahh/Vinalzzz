...
class PDFService {
  ...
  buildBAPBPDF(data, signatures, stream) {
    ...
    const infoData = [
      ...
      ['Tanggal Pengiriman', `: ${moment(data.delivery_date).format('DD MMMM YYYY')}`],
    ...
    if (signatures.vendorSignature) {
      try {
        const fileExists = fs.existsSync(signatures.vendorSignature);
        if (!fileExists) {
          throw new Error('Signature file not found');
        }

        const stats = fs.statSync(signatures.vendorSignature);
        if (stats.size === 0) {
          throw new Error('Signature file is empty');
        }

        console.log('📄 Loading vendor signature:', {
          path: signatures.vendorSignature,
          size: stats.size,
          exists: fileExists
        });
        ...
        console.log('✅ Vendor signature loaded successfully');
      } catch (err) {
        console.error('❌ Error loading vendor signature:', err.message);
        doc.fontSize(9)
           .fillColor('#DC2626')
           .text('(Tanda tangan error)', leftX, signatureY + 50, { 
             align: 'center',
             width: 120 
           })
           .fillColor('#000000');
    ...
    if (signatures.picGudangSignature) {
      try {
        const fileExists = fs.existsSync(signatures.picGudangSignature);
        if (!fileExists) {
          throw new Error('Signature file not found');
        }

        const stats = fs.statSync(signatures.picGudangSignature);
        if (stats.size === 0) {
          throw new Error('Signature file is empty');
        }

        console.log('📄 Loading PIC signature:', {
          path: signatures.picGudangSignature,
          size: stats.size,
          exists: fileExists
        });
        ...
        console.log('✅ PIC signature loaded successfully');
      } catch (err) {
        console.error('❌ Error loading PIC signature:', err.message);
        doc.fontSize(9)
           .fillColor('#DC2626')
           .text('(Tanda tangan error)', rightX, signatureY + 50, { 
             align: 'center',
             width: 120 
           })
           .fillColor('#000000');
